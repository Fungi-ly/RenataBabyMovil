$(function () {
  ...
  button.addEventListener('click', () => {
    ...
    const colorElement = button.parentElement.querySelector('input[name="product-color"]:checked');
  ...
  function updateCartSummary() {
    ...
    cart.forEach(item => {
      ...
      li.textContent = `${item.name} (${item.size}${item.color ? `, ${item.color}` : ''}) x ${item.quantity} - $${item.price * item.quantity}`;
